...
int main(int argc, char **argv)
{
	...
	u_int i=0;
	int res;
    const char filename[] = "./internet_trace.pcap";
...
printf("%d\n", sizeof(gtphdr));
	...
	while((res = pcap_next_ex(fp, &header, &pkt_data)) >= 0)
	{
        ...
        iph = (struct ip*)(pkt_data+iph->ip_hl*4+sizeof(struct udphdr));
        ...
        if (ether_type == ETHERTYPE_IP)
        {
            ...
            // iph = (struct ip*)(pkt_data + iph->ip_hl * 4);
